...
/**
 * This program is the starting point for Slenderman the game.
 * NOTE THAT THIS BUILD ONLY WORKS ON WINDOWS BECAUSE OF SOUND.
 * 
...
/*
	A billboard represents either a Page, 
	Tree, or Grass derived class, which has
	the basics of position, texture, translation,
	and scale but also additive ones such as
	yTranslate and widthRatio to center the billboards
	vertically while also maintaining the image size's
	proportions.
...
// grass class that inherits from the Billboard class
struct Grass: public Billboard {};

// Tree class that inherits from the Billboard class
struct Tree: public Billboard {};

// This class holds the information of a Page object
// which is also considered a 'billboard' but has its
// own rendering that revolves around a parent tree
...
class Viewer : public Window {
  public:
	...
	/*
	* Initializes the flashlight texture
	*/
	...
	/*
	*	Initializes the meshes of the program, i.e. Slenderman and flashlight
	*/
	void initModels() {
	...
	*/
	...
	/**
	*	This return the points of vectors at each cell grid in a
	* square x square box with the gridPointIdx being the center.
	*
	*/
	...
	/*
	*  This checks whether a given point is in the same neighborhood as
	*  as any other point that was already inserted into the grid, i.e.,
	*  the distance between the point and any other point in the grid
	*  is less than the minDist given.
	*/
	...
	/*
		This returns whether the point is in the 2D grid or not
		in the 2D grid given by the coordinates of the plane
	*/
	...
	/*
		This initializes the billboards such as the grass and trees, loading
		their assets and randomizing their location on the grid.
	*/
	void initBillboards() {

		// this is used to init the grass textures, but they were laggy
		// and unnecessary imo
		Image img;
		float grassRatios[4];
		...
		// this is to init the tree textures
		float treeRatios[2];
		...
		// this is the start of Poisson's disk algorithm
	...
	/*
	* This draws the billboards and assets, so that they 
	* are sorted.
	*/
	...
	/*
		This sets up the assets necessary to make the scene.
		This sets up the shaders, lighting information, plane
		texture, camera info, player info, slenderman info,
		and combining the rendering items so that they can 
		be sorted.
	*/
    void setup() {
	...
	// Checks whether there is an error with any sound function
	...
	// This responds to right mouse button drag so that the player can look around
	...
	*/
	...
	/*
		This checks if the player is ever looking at Slenderman,
		and damages the player if they are. This looking calcluation
		is done by finding the angle between the player's forward direction
		and the vector from the player to Slenderman using dot product.
	...
	*/
	...
	// Checks if a player ic lose to a page or not to be collected
	...
	// checks to see if the player has collected 8 pages yet
	void isWin() {
	...
	// checks to see if the player has lost all of their health
	void isLose() {
    ...
    void keyDown(int key, int mods) {
		...
		// turns off and on the spotlight by changing the lighting information
		if (key == GLFW_KEY_F) {
    ...
    // updates the targetPos to move the character
	// when they are holding down WASD
	...
	// Updates the lookPos based on how the player moves the camera
    void updateLookPos() {
	...
	// Initializes the shader information of each object given these paramters
	// Texture of the item can be specified, along with their uv, if you want to use their alpha
	// and if you want fog to affect it.
	...
	// For the lose screen, Slenderman will randomly glitch at a random time and play
	// a static sound
	...
	// This is the game loop where everything is updated based on gameStatus
    void draw() {
	...
	// This lerps the player movement, so the movement is smoother
	void lateUpdate() {
  ...
  protected:
	...
	// items to be rendered by sorting
